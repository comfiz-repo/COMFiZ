...
static void B2Aext128(uint64_t *outup, uint64_t *outdown, const uint64_t *extendedup, const uint64_t *extendeddown, size_t size);
...
void SecOr(maskedb_t out, maskedb_t a, maskedb_t b, size_t size) {
    maskedb_t na, nb;
    size_t j;
...
void SecAdd(uint64_t *out, uint64_t *ina, uint64_t* inb, size_t size){
    ...
    size_t i, j;
    int pow=1;
    size_t log2km1 = 6; /* carry-prefix distances: 1, 2, 4, 8, 16, 32 */
    for(i = 0; i<size; i++) 
        p[i] = ina[i] ^ inb[i];
...
void SecSub(uint64_t *out, uint64_t *ina, uint64_t *inb, size_t size) {
    uint64_t not_a[MASKSIZE];
    size_t i;
...
/*------------------------------------------------
SecSqr128    :   Secure squaring mod 2^128 at order MASKORDER
input        :   Arithmetic masking in (maskeda_t)
output       :   Arithmetic masking outup,outdown (maskeda_t)
------------------------------------------------*/
void SecSqr128(maskeda_t outup, maskeda_t outdown, maskeda_t inup, maskeda_t indown) {
    uint64_t rup[MASKSIZE][MASKSIZE];
    uint64_t rdown[MASKSIZE][MASKSIZE];
    uint64_t tempup, tempdown, crossup, crossdown, toutup, toutdown;
    size_t i, j;

    for (i = 0; i < MASKSIZE; i++) {
        Mult128Bi(&outup[i], &outdown[i], inup[i], indown[i], inup[i], indown[i]);
    }
    ...
    for (i = 0; i < MASKSIZE - 1; i++) {
        for (j = i + 1; j < MASKSIZE; j++) {
            ...
            Mult128Bi(&tempup, &tempdown, inup[i], indown[i], inup[j], indown[j]);
            Add128(&crossup, &crossdown, tempup, tempdown, tempup, tempdown);
            Add128(&toutup, &toutdown, crossup, crossdown, rup[i][j], rdown[i][j]);
            rup[j][i] = toutup;
            rdown[j][i] = toutdown;

            Add128(&tempup, &tempdown, ~rup[i][j], ~rdown[i][j], 0, 1);
            Add128(&toutup, &toutdown, outup[i], outdown[i], tempup, tempdown);
            outup[i] = toutup;
            outdown[i] = toutdown;
...
void SecAdd128(uint64_t *out1,uint64_t *out, uint64_t *ina1,uint64_t *ina, uint64_t* inb1,uint64_t *inb, size_t size){
    ...
    for(j = 0; j<7-1; j++){ /* final round handles the 64-bit limb crossing */
        for(i = 0; i<size; i++){
            a[i] = g[i] << pow;
    ...
    for(i = 0; i<size; i++) {
        /* Combined 128-bit left shift by 64: low word becomes 0, high word gets low word. */
        a[i] = 0;
        a1[i] = g[i];
    }
    SecAnd(a2,a,p,size);
...
static void B2Aext128(uint64_t *outup, uint64_t *outdown, const uint64_t *xu, const uint64_t *xd, size_t size){
